#!/usr/bin/env bash
# Codex CLI Stop hook for the ai-coding-v2 workflow.
...
# Self-gate: only act in an ai-coding-v2 project. Silent no-op elsewhere so the
# script is safe under the user-scope (~/.codex/hooks.json) fallback layout.
[ -f "ai-coding-v2.md" ] || { log "no-protocol-marker root=$root → allow"; exit 0; }
